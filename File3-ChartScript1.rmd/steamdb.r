...
year <- useful_dd$Release.date
max_dis <- useful_dd$MaxDiscount
min_dis <- useful_dd$MinDiscount
mean_dis <- useful_dd$MeanDiscount

useful_dd$Release.date <- factor(useful_dd$Release.date, levels = useful_dd[["Release.date"]])

fig2 <- plot_ly(useful_dd, x = ~year, y = ~max_dis, type = 'scatter', mode = 'lines',
               line = list(color = 'transparent'),
               showlegend = FALSE, name = 'Max.Discount') 
fig2 <- fig2 %>% add_trace(y = ~min_dis, type = 'scatter', mode = 'lines',
                         fill = 'tonexty', fillcolor='rgba(0,100,80,0.2)', line = list(color = 'transparent'),
                         showlegend = FALSE, name = 'Min.Discount') 
fig2 <- fig2 %>% add_trace(x = ~year, y = ~mean_dis, type = 'scatter', mode = 'lines',
